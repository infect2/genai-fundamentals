...
│   │   ├── fms/                # FMS 도메인 에이전트
│   │   ├── tap/                # TAP! 도메인 에이전트
│   │   └── memory/             # Memory 도메인 에이전트 (사용자 정보 저장/조회)
│   ├── ontology/               # 통합 온톨로지
│   │   ├── __init__.py
...
WMS, TMS, FMS, TAP!, Memory 서비스를 위한 연합형 멀티 에이전트 시스템입니다.
...
│  ┌──────────────────────────────────────────────┐   │
│  │           Domain Router                       │   │
│  │   쿼리 분석 → 도메인 선택 (WMS/TMS/FMS/TAP/Memory) │   │
│  └──────────────────────────────────────────────┘   │
...
| **Memory** | 사용자 정보 저장/조회 | store_memory, recall_memory, list_memories |
...
```json
{
  "query": "배송 현황 알려줘",
  "session_id": "user123",
  "preferred_domain": "auto",      // auto | wms | tms | fms | tap | memory
...
| Memory | 기억, 저장, 기억해, 내 정보, 내 이메일, 내 차번호, remember, store, recall |
