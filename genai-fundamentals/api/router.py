...
CLASSIFICATION_PROMPT = """당신은 영화 데이터베이스 쿼리 분류 전문가입니다.
...
1. **cypher**: 특정 엔티티나 관계를 조회하는 쿼리
   - 특정 배우, 감독, 영화 제목으로 검색
   - 관계 기반 질문 (누가 출연했는지, 누가 감독했는지)
   - 정확한 이름/제목이 언급된 경우
   - 예시: "톰 행크스가 출연한 영화는?", "매트릭스의 감독은?", "액션 장르 영화 목록"

2. **vector**: 시맨틱/의미 기반 검색이 필요한 쿼리
   - 줄거리, 분위기, 테마 기반 검색
   - 유사한 영화 찾기
   - 설명이나 느낌으로 검색
   - 예시: "슬픈 영화 추천해줘", "우주를 배경으로 한 영화", "반전이 있는 스릴러"

3. **hybrid**: 시맨틱 검색 + 필터/조건 조합
   - 의미 검색과 특정 조건 모두 필요
   - 예시: "90년대 액션 영화 중 평점 높은 것", "톰 행크스 영화 중 감동적인 것"

4. **llm_only**: 영화 DB 조회가 필요 없는 일반 질문
   - 영화 일반 상식, 정의
   - 인사말, 감사 표현
   - DB에 없는 정보 요청
